...
class SessionManager:
    ...
    def complete_session(cls, session_id):
        ...
        Also generates assessment if content has transcript
        
        Args:
            session_id: StudySession ID
            
        Returns:
            dict with session summary and assessment info
        """
        try:
            session = StudySession.objects.get(id=session_id)
        except StudySession.DoesNotExist:
        ...
        if session.is_completed:
            # Allow re-completing gracefully — return success instead of error
            try:
